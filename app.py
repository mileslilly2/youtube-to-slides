...
def images_to_ppt(directory, ppt_file):
  ...
  for image_file in images:
    slide_layout = prs.slide_layouts[
      5]  # Use the blank slide layout (layout 5)
    ...
    # Add the image
    slide.shapes.add_picture(os.path.join(directory, image_file), left, top,
                             width, height)
...
def download_youtube_as_mp4(video_url, resolution):
  ...
  video_stream = youtube.streams.filter(progressive=True,
                                        file_extension='mp4',
...
def extract_frames_cv2(video_path, seconds):
  ...
  while vidcap.isOpened():
    ...
    if frame_id % multiplier == 0:
      ...
      cv2.imwrite(os.path.join(directory_name, f"frame{frame_number}.jpg"),
                  frame)
